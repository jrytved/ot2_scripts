...
twintec_text = """
{
	...
	"brand": {
		"brand": "Eppendorf twin.tec",
		"brandId": []
	},
	"metadata": {
		"displayName": "Eppendorf Twin.tec 96 Well Plate 150 µL",
	...
	"parameters": {
		...
		"loadName": "eppendorftwin.tec_96_wellplate_150ul"
	},
...
integra_reservoir_text = """
{
	...
	"metadata": {
		"displayName": "Integra 100mL Reservoir",
	...
	"parameters": {
		...
		"isTiprack": false,
		"isMagneticModuleCompatible": false,
		"loadName": "integra_100ml_reservoir"
	},
	"namespace": "custom_beta",
	"version": 1,
	"schemaVersion": 2,
...
bca_plate_text = """
{
	...
	"metadata": {
		"displayName": "Greiner 96 Flat Bottom BCA Plate",
		"displayCategory": "wellPlate",
		"displayVolumeUnits": "µL",
		"tags": []
	},
...
def pipette_method_a(PIPETTE, SOURCE: str, DEST_LIST: [str], VOLUME: int):
	...
	for DEST in DEST_LIST:
		...
		PIPETTE.move_to(DEST.bottom(1))
		PIPETTE.dispense(VOLUME, DEST.bottom(0.8))
		PIPETTE.blow_out(DEST.bottom(3))
		PIPETTE.drop_tip()



def pipette_method_b(PIPETTE, SOURCE: str, DEST_LIST: [str], VOLUME: int):
	
	# Pre wet tips, take new tip for each transfer, add air gap

	AIR_GAP = 3

	for DEST in DEST_LIST:
		PIPETTE.pick_up_tip()
		PIPETTE.mix(2, VOLUME+AIR_GAP, SOURCE)
		PIPETTE.aspirate(VOLUME, SOURCE)
		PIPETTE.air_gap(AIR_GAP)
		PIPETTE.move_to(DEST.bottom(1))
		PIPETTE.dispense(VOLUME+AIR_GAP, DEST.bottom(0.8))
		PIPETTE.blow_out(DEST.bottom(3))
		PIPETTE.drop_tip()

def pipette_method_c(PIPETTE, SOURCE: str, DEST_LIST: [str], VOLUME: int):
	
	# Pre wet tips, use the same tip for both transfers

	AIR_GAP = 3
	assert AIR_GAP*2+VOLUME*2 <= 20, "Total volume exceeds pipette capacity."
	...
	for DEST in DEST_LIST:
		PIPETTE.move_to(DEST.bottom(1))
		PIPETTE.dispense(VOLUME+AIR_GAP)
...
STANDARD_TRANSFER_MAP = [
	...
	{"Source": "A1", "Destinations": ["A1", "A2", "A3"], "FUN": pipette_method_a}
]


SAMPLE_TRANSFER_MAP = [

	{"Source": "A1","Destinations": ["A4", "A5"], "FUN": pipette_method_a},
	{"Source": "A1","Destinations": ["A6", "A7"], "FUN": pipette_method_b},
	{"Source": "A1","Destinations": ["A8", "A9"], "FUN": pipette_method_c},
]
...
INCUBATION_TIME_SECONDS = 60*INCUBATION_TIME_MINUTES

# Absolute flow rates in uL/s
# These flow rates are optimized for pipetting with greatest precision and accuracy.
P20_ASPIRATE_FLOW_RATE = 4
P20_DISPENSE_FLOW_RATE = 4

P300_ASPIRATE_FLOW_RATE = 30
P300_DISPENSE_FLOW_RATE = 35
...
def run(protocol: protocol_api.ProtocolContext):
	...
	module_name="heaterShakerModuleV1", location="3"
	)
	...
	# Specify the flat bottom adapter is used on the temp module
	...
	protein_sample_plate = temp_adapter.load_labware_from_definition(twintec_json)				
	...
	pipette_20 = protocol.load_instrument(
		"p20_multi_gen2", mount = "right", tip_racks = [tips_20]
	)
	...
	protocol.comment("Waiting for temperature module to cool and for heater-shaker to heat.")

	if USE_TEMP_MOD:
		temp_mod.set_temperature(celsius=TM_TEMP) 
	...
	for TRANSFER in STANDARD_TRANSFER_MAP:
		...
		FUN(pipette_20, SOURCE, DEST_LIST, VOLUME = BSA_STANDARD_VOL)
	...
	DEST_LIST = [bca_assay_plate[well] for well in DILUENT_RECEIVING_WELLS]

	pipette_20.mix(3, DILUENT_VOL+2, diluent_reservoir["A1"].bottom(2))

	for DEST in DEST_LIST:
		pipette_20.aspirate(DILUENT_VOL, diluent_reservoir["A1"].bottom(2))
		pipette_20.air_gap(2)
		pipette_20.move_to(DEST.bottom(1.0))
		pipette_20.dispense(DILUENT_VOL+2, DEST.bottom(0.8))
		pipette_20.blow_out(DEST.bottom(3))
	...
	for TRANSFER in SAMPLE_TRANSFER_MAP:
		...
		FUN(pipette_20, SOURCE, DEST_LIST, VOLUME = SAMPLE_VOL)
	...
	protocol.delay(INCUBATION_TIME_SECONDS)
